...
async def compose_email(draft, language):
    subject = await create_subject(draft)
    summary = await summarize_summary(draft)
    sentiment = await get_sentiment(draft)
...
async def summarize_summary(draft):
...
async def send_email_handler(
    ...
    mail_service = FastMail(mail_service)
    if not mail_service:
